...
// [EDO] >>>
std::vector<std::vector<float>> TreeObsForRailEnv::get_many(
// [EDO] .
    ...
    */
    ...
    // [EDO] <<< - removed as I don't need forest
    // Forest forest;
    ...
    // [EDO] >>>
    std::vector<std::vector<float>> tree_obs;
    for (auto handle : handles) {
        auto agent_tree_obs = this->get(agents_loader.agents[handle]);
        tree_obs.push_back(agent_tree_obs);
    }
    return tree_obs;
    // [EDO] .
}
...
// [EDO] >>>
std::vector<float> TreeObsForRailEnv::get(const Agent& agent) {
// [EDO] .
    std::vector<float> tree_obs;
    ...
    // [EDO] <<< - removed as I want to return tree_obs as a unique vector
    // tree_obs.push_back(root);
    // [EDO] >>>
    tree_obs.reserve(this->observation_dim * this->max_nodes);
    tree_obs.insert(tree_obs.end(), root.begin(), root.end());
    // [EDO] .
    ...
    // [EDO] >>>
    for (int idx_node=1; idx_node < this->max_nodes; idx_node++) {
    // [EDO] .
        ...
        // [EDO] >>>
        tree_obs.insert(tree_obs.end(), branch_observation.begin(),
                        branch_observation.end());
        // [EDO] .
    ...
    // [EDO] <<< - removed as I don't need node_order and edge_order
